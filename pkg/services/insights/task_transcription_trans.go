...
type TranscriptionWithTranslationTask struct {
	conf   *config.ServiceConfig
	creds  *config.CredentialsConfig // Store credentials
	logger *logrus.Entry
}

func NewTranscriptionWithTranslationTask(conf *config.ServiceConfig, creds *config.CredentialsConfig, logger *logrus.Entry) (insights.Task, error) {
	return &TranscriptionWithTranslationTask{
		conf:   conf,
		creds:  creds,
		logger: logger,
	}, nil
...
func (t *TranscriptionWithTranslationTask) Run(ctx context.Context, audioStream <-chan []byte, roomID, identity string, options []byte) error {
	...
	stream, err := provider.CreateTranscriptionWithTranslation(ctx, roomID, identity, options)
	if err != nil {
		return err
	}

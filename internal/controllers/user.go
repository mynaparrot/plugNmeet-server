...
// HandleApproveWaitingParticipant will use to approve user who is in waiting room
func HandleApproveWaitingParticipant(c *fiber.Ctx) error {
	roomId := c.Locals("roomId")
	isAdmin := c.Locals("isAdmin")
	...
	m := models.NewUserModel()
	req := new(models.ApproveWaitingParticipantReq)
	...
	req.RoomId = roomId.(string)
	err = m.ApproveWaitingParticipant(req)
	if err != nil {
		return c.JSON(fiber.Map{
			"status": false,
			"msg":    err.Error(),
